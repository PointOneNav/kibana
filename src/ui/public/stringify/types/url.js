...
export function stringifyUrl(Private) {
    ...
    Url.editor = {
        ...
        controller: function($scope, chrome) {
            ...
            const videoPattern = `${chrome.getBasePath()}/bundles/src/ui/public/stringify/icons/{{value}}.mp4`;

            this.samples = {
                a: ['john', '/some/pathname/asset.png', 1234],
                img: ['go', 'stop', ['de', 'ne', 'us', 'ni'], 'cv'],
                video: ['go', 'stop', ['de', 'ne', 'us', 'ni'], 'cv']
            };
            ...
            $scope.$watch('editor.formatParams.type', function(type, prev) {
                ...
                if (type === 'video' && type !== prev && !params.urlTemplate) {
                    params.urlTemplate = videoPattern;
                }
            });
        }
    };
